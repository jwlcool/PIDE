...
from propagate import get_neigbors
...
parser.add_argument('--embedding_dim', default=10, type=int, help='Number of dimensions of the dynamic embedding')
                ...
                if timestamp - tbatch_start_time > tbatch_timespan:
                    ...
                    with trange(len(lib.current_tbatches_user)) as progress_bar3:
                        for i in progress_bar3:
                            ...
                            total_interaction_count =total_interaction_count+ len(lib.current_tbatches_interactionids[i])
                            ...
                            user_neighbors = {}
                            item_neighbors = {}
                            userids = lib.current_tbatches_user[i]
                            itemids = lib.current_tbatches_item[i]

                            for id, value in enumerate(tbatch_userids):
                                user = str(value.cpu().numpy())
                                user = 'u' + user
                                user_neighbors[user] = get_neigbors(kg, user, depth=1)[1]
                            for id, value in enumerate(tbatch_itemids):
                                item = str(value.cpu().numpy())
                                item = 'i' + item
                                item_neighbors[item] = get_neigbors(kg, item, depth=1)[1]
                            users=tbatch_userids.cpu().numpy()
                            items=tbatch_itemids.cpu().numpy()
                            for iter in user_neighbors:
                                for i in user_neighbors[iter]:
                                    item = int(i[1:])
                                    if item not in tbatch_itemids.cpu().numpy():
                                        item=torch.LongTensor([item]).cuda()
                                        itemids=torch.cat((itemids,item),dim=0)
                            for iter in item_neighbors:
                                for i in item_neighbors[iter]:
                                    user = int(i[1:])
                                    if user not in tbatch_userids.cpu().numpy():
                                        user=torch.LongTensor([user]).cuda()
                                        userids=torch.cat((userids,user),dim=0)
                            # PROJECT USER EMBEDDING TO CURRENT TIME
                            u_embedding_input = user_embeddings[userids]
                            i_embedding_input = item_embeddings[itemids]

                            user_embedding_input = user_embeddings[tbatch_userids,:]
                            item_embedding_input = item_embeddings[tbatch_itemids, :]
                            user_projected_embedding = model.forward(kg,user_embedding_input,item_embedding_input,u_embedding_input,i_embedding_input,userids,itemids,tbatch_userids,tbatch_itemids, timediffs=user_timediffs_tensor, features=feature_tensor, select='project')
                            ...
                            # item_embedding_input = item_embeddings[tbatch_itemids,:]
                            loss =loss+ MSELoss(predicted_item_embedding, torch.cat([item_embedding_input, item_embedding_static[tbatch_itemids,:]], dim=1).detach())

                            # UPDATE DYNAMIC EMBEDDINGS AFTER INTERACTION
                            user_embedding_output,i_embeddings = model.forward(kg,user_embedding_input,item_embedding_input,u_embedding_input,i_embedding_input,userids,itemids,tbatch_userids,tbatch_itemids, timediffs=user_timediffs_tensor, features=feature_tensor, select='user_update')
                            item_embedding_output,u_embeddings = model.forward(kg,user_embedding_input,item_embedding_input,u_embedding_input,i_embedding_input,userids,itemids,tbatch_userids,tbatch_itemids, timediffs=item_timediffs_tensor, features=feature_tensor, select='item_update')

                            # item_embeddings[tbatch_itemids,:] = item_embedding_output
                            # user_embeddings[tbatch_userids,:] = user_embedding_output
                            item_embeddings[itemids] = i_embeddings
                            user_embeddings[userids] = u_embeddings
                            ...
                            loss = loss + MSELoss(item_embedding_output, item_embedding_input.detach())
                            loss = loss + MSELoss(user_embedding_output, user_embedding_input.detach())

                            # CALCULATE STATE CHANGE LOSS
                            if args.state_change:
                                loss =loss + calculate_state_prediction_loss(model, tbatch_interactionids, user_embeddings_timeseries, y_true, crossEntropyLoss)

...
arg, _ = opts.parse_known_args()
if torch.cuda.is_available() and arg.gpu >= 0:
    DEVICE = torch.device('cuda:' + str(arg.gpu))
    print('use gpu indexed: %d' % arg.gpu)
else:
...
kg=nx.Graph()
for iter in range(train_end_idx):
...
with trange(train_end_idx, test_end_idx) as progress_bar:
    for j in progress_bar:
        ...
        # if j >= test_start_idx:
        if userid not in user_true_item:
            user_true_item[userid]=[]
        if itemid not in user_true_item[userid]:
            user_true_item[userid].append(itemid)
        if userid not in user_rec_5:
            user_rec_5[userid] = []
        if userid not in user_rec_10:
            user_rec_10[userid] = []
        if userid not in user_rec_20:
            user_rec_20[userid] = []
        ...
        user_projected_embedding = model.forward(u_embedding_input,i_embedding_input,user_set,item_set,user_embedding_input, item_embedding_previous, timediffs=user_timediffs_tensor, features=feature_tensor, select='project')
        ...
        # CALCULATE DISTANCE OF PREDICTED ITEM EMBEDDING TO ALL ITEMS 当前预测item的embedding与所有item距离
        ...
        euclidean=euclidean_distances.data.cpu().numpy()
        rank_item=rankdata(euclidean)

        user_rec_5 = user_topK(5, user_rec_5, userid, rank_item)
        user_rec_10 = user_topK(10, user_rec_10, userid, rank_item)
        user_rec_20 = user_topK(20, user_rec_20, userid, rank_item)
        # CALCULATE RANK OF THE TRUE ITEM AMONG ALL ITEMS 当前预测item的embedding与实际交互item距离
        ...
        user_embedding_output,i_embedding_output = model.forward(u_embedding_input,i_embedding_input,user_set,item_set,user_embedding_input, item_embedding_input, timediffs=user_timediffs_tensor, features=feature_tensor, select='user_update')
        item_embedding_output,u_embedding_output = model.forward(u_embedding_input,i_embedding_input,user_set,item_set,user_embedding_input, item_embedding_input, timediffs=item_timediffs_tensor, features=feature_tensor, select='item_update')
...
HR_5 = computeHR(user_rec_5, user_true_item)
HR_10 = computeHR(user_rec_10, user_true_item)
HR_20 = computeHR(user_rec_20, user_true_item)
MAR=mar/(test_end_idx-train_end_idx)
ranks = test_ranks
mrr = np.mean([1.0 / r for r in ranks])
rec10 = sum(np.array(ranks) <= 10)*1.0 / len(ranks)
...
fw.write("MAR:" + str(MAR) + ' ' +"HR@5:" + str(HR_5) + ' ' +"HR@10:" + str(HR_10) + ' '+"HR@20:" + str(HR_20) + ' '+"\n")
fw.flush()
fw.close()

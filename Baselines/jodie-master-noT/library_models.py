...
class JODIE(nn.Module):
    def __init__(self, args, num_features, num_users, num_items):
        ...
        self.inter_linear=nn.Linear(args.embedding_dim,args.embedding_dim)
        self.neibor_linear=nn.Linear(args.embedding_dim,args.embedding_dim)
        
    def forward(self, u_embedding_input,i_embedding_input,user_set,item_set,
                user_embeddings, item_embeddings, timediffs=None, features=None, select=None):
        if select == 'item_update':
            u_embedding_output = u_embedding_input.clone()
            input1 = torch.cat([user_embeddings, timediffs, features], dim=1)
            item_embedding_output = self.item_rnn(input1, item_embeddings)

            userid = user_set[0]
            for id, value in enumerate(user_set):
                if value != userid:
                    neibor_embedding = u_embedding_input[id]  # 获取邻居节点嵌入
                    inter_sim = neibor_embedding * item_embeddings  # 点乘
                    ...
                    u_embedding_output[id] = nn.Sigmoid()(neiborghhood_embedding)  # 激活函数

            return F.normalize(item_embedding_output),u_embedding_output

        elif select == 'user_update':
            i_embedding_output = i_embedding_input.clone()
            input2 = torch.cat([item_embeddings, timediffs, features], dim=1)
            user_embedding_output = self.user_rnn(input2, user_embeddings)

            itemid=item_set[0]
            for id,value in enumerate(item_set):
                if value!=itemid:
                    neibor_embedding = i_embedding_input[id]  # 获取邻居节点嵌入
                    inter_sim = neibor_embedding * user_embeddings  # 点乘
                    inter_sim = self.inter_linear(inter_sim)
                    neibor_embedding = self.neibor_linear(neibor_embedding)
                    neiborghhood_embedding = neibor_embedding + inter_sim
                    i_embedding_output[id] = nn.Sigmoid()(neiborghhood_embedding)  # 激活函数



            return F.normalize(user_embedding_output),i_embedding_output
...
def user_topK(k, user_rec, user,ranks):

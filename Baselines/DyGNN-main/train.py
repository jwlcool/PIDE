...
def get_args():
    ...
    parser.add_argument('-data', '--dataset', type = str,
                        help = 'which dataset to run', default = 'music')
...
def train(args, data, num_nodes, model_save_dir):
    ...
    for epoch in range(num_iter):
        ...
        HR = get_HitRatio(train_data,test_data,head_reps, tail_reps, head_node2candidate, tail_node2candidate,device)
        ...
        print('MAR:',head_ranks_numpy[150:200].sum()/len(head_ranks_numpy))
...
def get_HitRatio(train_data,test_data,head_reps,tail_reps,head_node2candidate,tail_node2candidate,device):
    user_list=[]
    item_list=[]
    for node in head_node2candidate:
        node_tensor = head_reps(torch.LongTensor([node]).to(device)).view(-1, 1)
        candidates = list(head_node2candidate[node])

        candidates.append(test_data)

        length = len(candidates)

        candidate_tensors = head_reps(torch.LongTensor(candidates).to(device))
...
if __name__ == '__main__':
    ...
    if args.dataset == 'music':
        data = Temporal_Dataset('Dataset/music/music.txt', 1, 2)
        num_nodes = 4432
        model_save_dir = model_save_dir + 'music/'
        print('Train on music dataset')
        train(args, data, num_nodes, model_save_dir)
    else:

...
def load_data_as_graph(path, weight_idx, time_idx):
    '''
...
def create_embedding_and_training_data_old(g, train_edges_fraction=cmd_args.train_edge):
...
def create_embedding_and_training_data(g, train_edges_fraction=cmd_args.train_edge):
    '''
...
def main():
    path =cmd_args.train_file
    contact_g =  load_data_as_graph(path=path, weight_idx=5, time_idx=2)
    embedding_edges, pos_edges, neg_edges = create_embedding_and_training_data(contact_g, train_edges_fraction=cmd_args.train_edge)

    save_path = cmd_args.save_dir

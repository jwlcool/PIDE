...
class TimeGraph:
    def __init__(self, edgelist, time_prop_name):
        ...
        # self.G = nx.Graph()
        # self.G=self.create_kg(self.G,edgelist)
    ...
    # def create_kg(self,kg,edge_list):
    #     for i in edge_list:
    #         u=i[0]
    #         v=i[1]
    #         time=i[2]['time']
    #         kg.add_node(u)
    #         kg.add_node(v)
    #         kg.add_edge(u,v, time=time)
    #         kg.add_edge(v,u, time=time)
    #     return kg
    ...
    def _temporal_walk(self, start_edge=None, t=None, C=None): # Algorithm 2 from CTDNE
        ...
        for p in range(1, min(self.L, C)):
            ...
            for u, v, prop in G.out_edges(nbunch=curr_node, keys=False, data=True):
                if prop[prop_name] >= t:
            ...
            # for i in adj:
            #     if adj[i]['time'] >= t:
            #         neighbor_candidates.append((i, adj[i]['time']))
...
def main():
    ...
    timeG.set_temporal_walk_params(beta=beta, D=cmd_args.embedding_size, omega=omega, L=L)

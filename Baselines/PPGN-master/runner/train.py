    ...
    with tf.Session() as sess:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            try:
                while True:
                    predictions_s, labels_s, predictions_t, labels_t = model.step(sess)
                    hr_s, mrr_s, ndcg_s,mar_s = evaluate(predictions_s, labels_s)
                    hr_t, mrr_t, ndcg_t,mar_t = evaluate(predictions_t, labels_t)
                    MAR_t.append(mar_t)
                    MAR_s.append(mar_s)
                    HR_s.append(hr_s)
                    MRR_s.append(mrr_s)
                ...
                mar_t=np.array(MAR_t).mean()
                mar_s = np.array(MAR_s).mean()
                mar_t_list.append(mar_t)
                mar_s_list.append(mar_s)
                ...
                print("Epoch %d, finish testing " % epoch + "took: " +
                      time.strftime("%H: %M: %S", time.gmtime(time.time() - start_time)) + ';')
                print('Epoch %d, %s MAR_t is %.4f,HR is %.4f, MRR is %.4f, NDCG is %.4f;' %
                      (epoch, dataset_s.name, mar_t,hr_s, mrr_s, ndcg_s))
                print('Epoch %d, %s MAR_t is %.4f,HR is %.4f, MRR is %.4f, NDCG is %.4f;' %
                      (epoch, dataset_t.name, mar_s,hr_t, mrr_t, ndcg_t))

        print('=' * 30 + 'Finish training' + '=' * 30)
        print('%s best MAR_t is %.4f,best HR is %.4f, MRR is %.4f, NDCG is %.4f;' %
              (dataset_s.name,max(mar_t_list), max(hr_s_list), max(mrr_s_list), max(ndcg_s_list)))
        print('%s best MAR_s is %.4f,best HR is %.4f, MRR is %.4f, NDCG is %.4f;' %
              (dataset_t.name, max(mar_s_list),max(hr_t_list), max(mrr_t_list), max(ndcg_t_list)))
